...
export default function ContactInfoSection() {
  return (
    <section className="bg-white py-16 lg:py-20" aria-labelledby="contact-info-heading">
            ...
            {CONTACT_INFO.map((item) => {
              const CardInner = (
                <>
                  ...
                  <h3 className="font-semibold text-[#1E293B] text-sm mb-2">{item.title}</h3>
                  {item.lines.map((line) => (
                    <p key={line} className="text-[#64748B] text-xs leading-relaxed">
                      {line}
                    </p>
                  ))}
                </>
